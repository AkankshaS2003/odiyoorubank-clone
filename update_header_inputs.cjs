          ...
          <div className="bg-[#ED7F1E] rounded-t-2xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between mb-8 -mt-8 md:-mt-12 -mx-8 md:-mx-12 print:m-0 print:p-4 print:rounded-none gap-4 md:gap-0">
            ...
            <div className="text-right text-white text-[10px] md:text-xs font-bold space-y-3 w-full md:w-56 shrink-0 flex flex-col items-end mt-4 md:mt-0">
              ...
              <div className="flex flex-col items-end gap-1 w-full">
                ...
                <input type="text" value={typeof user !== 'undefined' ? (user?.customerId || '') : ''} readOnly className="w-32 bg-white/20 rounded px-2 py-1 outline-none text-center text-white border border-white/10 placeholder-white/60 font-bold tracking-wide" />
              </div>
              <div className="flex justify-end items-center gap-2 w-full">
                <span className="opacity-90">Account No:</span> 
                <input type="text" value={typeof user !== 'undefined' ? (user?.accountNumber || '— — — —') : '— — — —'} readOnly className="w-32 border-b border-white/40 outline-none bg-transparent text-center text-white placeholder-white/60 focus:border-white transition-colors tracking-widest font-bold" />
              </div>
            </div>
          </div>`;
