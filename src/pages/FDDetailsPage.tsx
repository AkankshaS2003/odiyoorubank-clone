...
export const FDDetailsPage = ({ appId, setCurrentTab }: { appId: string, setCurrentTab: (tab: string) => void }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const canvas = await html2canvas(element, { scale: 2, backgroundColor: '#FAF7F2', useCORS: true, windowWidth: element.scrollWidth, windowHeight: element.scrollHeight });
      ...
      // Center vertically
      const pdfY = (pdf.internal.pageSize.getHeight() - pdfHeight) / 2;
      
      pdf.addImage(imgData, 'PNG', 0, pdfY > 0 ? pdfY : 0, pdfWidth, pdfHeight);
  ...
  const isValid = (val: string) => {
    ...
    return lower !== 'n/a' && lower !== 'not provided' && lower !== 'address not provided';
  };
      ...
      <div className="space-y-8 print:space-y-4">
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 px-2">
                ...
                {isValid(displayDOB) && <LabelValue label="Date of Birth" value={displayDOB} />}
                ...
                {isValid(displayMobile) && <LabelValue label="Mobile Number" value={displayMobile} />}
                {isValid(displayEmail) && <LabelValue label="Email Address" value={displayEmail} />}
                {isValid(displayPan) && <LabelValue label="PAN Number" value={displayPan} />}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:block print:space-y-4">
          <section className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 print:rounded-none print:border-none print:shadow-none print:p-0">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <LabelValue label="Deposit Date" value={formatDate(fd.depositDate)} />
              <LabelValue label="Interest Rate" value={`${fd.interestRate}% p.a.`} />
              <LabelValue label="Tenure" value={formData.depositPeriod || `${fd.tenureMonths} Months`} />
              <LabelValue label="Maturity Date" value={formatDate(fd.maturityDate)} />
          ...
          <section className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 print:rounded-none print:border-none print:shadow-none print:p-0">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <LabelValue label="Payment Date" value={mainTxn.date ? formatDate(mainTxn.date) : formatDate(fd.depositDate)} />
            ...
            <div className="mt-8 p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-700">Processed by Automated Banking System on {formatDate(application.processedAt || fd.depositDate)}</p>
              </div>
            </div>
          </section>
        </div>
        ...
        {fd.status === 'Active' && (
          <section className="mt-8 print:mt-16 print:break-before-page">
            ...
            <div ref={certificateRef} className="relative mx-auto max-w-4xl shadow-xl print:shadow-none overflow-hidden" 
                 style={{ 
                   backgroundColor: '#FAF7F2', // Medium vintage professional base
              ...
              <div className="relative z-10 px-8 text-center leading-[2.5] italic">
                <p className="text-lg md:text-xl text-slate-800 text-justify">
                  ...
                  , residing at <span className="border-b border-dashed border-slate-400 px-2">{displayAddress}</span>, 
                  ...
                  into the Fixed Deposit scheme under the FD Number <span className="font-bold font-mono text-[#0F4C81] tracking-wider text-xl px-2">{fd.fdNumber}</span>. 
                </p>
                <p className="text-lg md:text-xl text-slate-800 text-justify mt-4">
                  The deposit was initiated on <span className="font-bold px-2">{formatDate(fd.depositDate)}</span> and is contracted for a tenure of 
                  <span className="font-bold px-2">{formData.depositPeriod || `${fd.tenureMonths} Months`}</span>, yielding an interest rate of <span className="font-bold px-2">{fd.interestRate}% p.a.</span> 
                  (compounded {fd.compoundingFrequency?.toLowerCase() || 'quarterly'}). The said deposit shall mature on 
                  <span className="font-bold px-2">{formatDate(fd.maturityDate)}</span>, with an accumulated maturity value of 
              ...
              <div className="text-center mt-12 pb-4 relative z-10">
                <p className="text-xs text-slate-500 font-medium italic font-sans">
                  This certificate is computer generated on {formatDate(new Date())} and is legally binding subject to the terms and conditions of the Society.
                </p>
              </div>
            </div>
          </section>
