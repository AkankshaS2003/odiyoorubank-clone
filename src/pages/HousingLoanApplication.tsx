...
export const HousingLoanApplication: React.FC<HousingLoanApplicationProps> = ({ setCurrentTab }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.loanAmountRequired || !formData.propertyType || !formData.tenure) {
      alert("Please fill in Property Type, Loan Amount Required, and Tenure.");
      return;
    }
    ...
    try {
      ...
      if (coSignatureFile) images.coSignature = await fileToBase64(coSignatureFile);
    } catch (err) {
      ...
      <div className="bg-slate-50 min-h-[80vh] flex items-center justify-center p-4">
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl border border-slate-300 max-w-lg w-full text-center animate-scale-up">
          ...
          <p className="text-slate-500 mb-8">
            Your Housing Loan Application (No: {formData.applicationNo}) for ₹{Number(formData.loanAmountRequired).toLocaleString('en-IN')} has been successfully received.
          </p>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 text-left mb-8 space-y-3">
            ...
            <div className="flex items-center gap-3 text-sm font-medium text-slate-400 opacity-60">
              ...
              <span>Legal & Technical Verification</span>
            </div>
            <div className="flex items-center gap-3 text-sm font-medium text-slate-400 opacity-60">
              <div className="w-5 h-5 rounded-full border-2 border-slate-300"></div>
        ...
        <div className="bg-white p-8 md:p-12 shadow-2xl shadow-slate-200 border border-slate-300 print:shadow-none print:border-none print:p-2">
          ...
          <div className="mb-8 border border-slate-200 rounded-xl p-5 print:border-slate-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div className="lg:col-span-2"><SelectField label="Marital Status" name="maritalStatus" value={formData.maritalStatus} onChange={handleChange} options={['Single', 'Married', 'Divorced', 'Widowed']} /></div>
            </div>

            <div className="mb-6 print:hidden">
              <label className="block text-[10px] font-bold text-[#0F4C81] mb-1 uppercase tracking-wider">Applicant Photo Upload</label>
          ...
          <div className="mb-8 border border-slate-200 rounded-xl p-5 print:border-slate-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <InputField label="Existing EMI Obligations (₹)" name="existingEmis" type="number" value={formData.existingEmis} onChange={handleChange} />
            </div>
          </div>
          ...
          <div className="mb-8 border border-slate-200 rounded-xl p-5 print:border-slate-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <SelectField label="Property Type" name="propertyType" value={formData.propertyType} onChange={handleChange} options={['House Construction', 'House Purchase', 'Flat Purchase', 'Site Purchase', 'Home Renovation', 'Home Extension']} />
              <InputField label="Property Market Value (₹)" name="propertyValue" type="number" value={formData.propertyValue} onChange={handleChange} />
              ...
              <InputField label="Property Size (Sq.ft)" name="propertySize" value={formData.propertySize} onChange={handleChange} />
              <InputField label="Estimated Construction Cost (₹)" name="constructionCost" type="number" value={formData.constructionCost} onChange={handleChange} />
            </div>
          </div>
          ...
          <div className="mb-8 border border-slate-200 rounded-xl p-5 print:border-slate-400">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 print:hidden">
              <div>
                <label className="text-[10px] font-bold text-[#0F4C81] mb-1 uppercase block">Sale Deed / Agreement</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-[#0F4C81] mb-1 uppercase block">Approved Building Plan</label>
                <input type="file" accept=".pdf,image/*" onChange={e => setBuildingPlanFile(e.target.files?.[0] || null)} className="text-xs text-slate-500 w-full file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-[#EAF6FF] file:text-[#0F4C81]" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-[#0F4C81] mb-1 uppercase block">Tax Paid Receipt</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-[#0F4C81] mb-1 uppercase block">Property Photos</label>
                <input type="file" accept="image/*" onChange={e => setPropertyPhotosFile(e.target.files?.[0] || null)} className="text-xs text-slate-500 w-full file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-[#EAF6FF] file:text-[#0F4C81]" />
              </div>
            </div>
          ...
          <div className="mb-8 border border-slate-200 rounded-xl p-5 print:border-slate-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              ...
              <InputField label="Relationship" name="coRel" value={formData.coRel} onChange={handleChange} />
              <InputField label="Mobile Number" name="coMobile" value={formData.coMobile} onChange={handleChange} />
              <div className="lg:col-span-2"><InputField label="Occupation" name="coOccupation" value={formData.coOccupation} onChange={handleChange} /></div>
